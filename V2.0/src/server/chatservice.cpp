...
ChatService::ChatService()
{
    ...
    _msgHandlerMap.insert({REM_MSG, std::bind(&ChatService::remFriend, this, _1, _2, _3)});
    _msgHandlerMap.insert({MODPW_MSG, std::bind(&ChatService::modifypassword, this, _1, _2, _3)});
    // 群组业务管理相关事件处理回调注册
...
//删除好友业务
void ChatService::remFriend(const TcpConnectionPtr &conn, json &js, Timestamp)
{
    int userid = js["id"].get<int>();
    int friendid = js["friendid"].get<int>();
    _userModel.remote(userid, friendid);
}

//修改用户密码业务
void ChatService::modifypassword(const TcpConnectionPtr &conn, json &js, Timestamp)
{
    int userid = js["id"].get<int>();
    string password = js["password"];
    _userModel.updataPassword(userid, password);
}
